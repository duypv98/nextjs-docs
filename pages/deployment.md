...
- HTML files for pages using `getStaticProps` or [Automatic Static Optimization](/advanced-features/automatic-static-optimization)
...
- `.next/server/pages` – The HTML and JavaScript entry points prerendered from the server. The `.nft.json` files are created when [Output File Tracing](/advanced-features/output-file-tracing) is enabled and contain all the file paths that depend on a given page.
- `.next/server/chunks` – Shared JavaScript chunks used in multiple places throughout your application
- `.next/cache` – Output for the build cache and cached images, responses, and pages from the Next.js server. Using a cache helps decrease build times and improve performance of loading images

All JavaScript code inside `.next` has been **compiled** and browser bundles have been **minified** to help achieve the best performance and support [all modern browsers](/basic-features/supported-browsers-features).
...
- [Pages](/basic-features/pages) are automatically statically optimized, if possible
...
- [API Routes](/api-routes/introduction) are automatically optimized as isolated [Serverless Functions](https://vercel.com/features/infrastructure?utm_source=next-site&utm_medium=docs&utm_campaign=next-website) that can scale infinitely
- [Middleware](/middleware) are automatically optimized as [Edge Functions](https://vercel.com/features/edge-functions?utm_source=next-site&utm_medium=docs&utm_campaign=next-website) that have zero cold starts and boot instantly
...
- Support for [Image Optimization](/basic-features/image-optimization) with `next/image`
...
You can self-host Next.js with support for all features using Node.js or Docker. You can also do a Static HTML Export, which [has some limitations](/advanced-features/static-html-export).
...
If you’d like to do a static HTML export of your Next.js app, follow the directions on our [Static HTML Export documentation](/advanced-features/static-html-export).
...
> **Note:** There are also managed platforms that allow you to use a Dockerfile as shown in the [example above](/deployment#docker-image).

### Static Only

The following services support deploying Next.js using [`next export`](/advanced-features/static-html-export).
...
You can also manually deploy the [`next export`](/advanced-features/static-html-export) output to any static hosting provider, often through your CI/CD pipeline like GitHub Actions, Jenkins, AWS CodeBuild, Circle CI, Azure Pipelines, and more.
...
> **Note:** Not all serverless providers implement the [Next.js Build API](/deployment#nextjs-build-api) from `next start`. Please check with the provider to see what features are supported.
