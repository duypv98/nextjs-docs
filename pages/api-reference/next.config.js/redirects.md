...
When leveraging [`basePath` support](/api-reference/next.config.js/basepath) with redirects each `source` and `destination` is automatically prefixed with the `basePath` unless you add `basePath: false` to the redirect:
...
- Inside [API Routes](/api-routes/response-helpers), you can use `res.redirect()`.
- Inside [`getStaticProps`](/api-reference/data-fetching/get-static-props) and [`getServerSideProps`](/api-reference/data-fetching/get-server-side-props), you can redirect specific pages at request-time.
