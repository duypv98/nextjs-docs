...
- `params`: If this page uses a [dynamic route](/routing/dynamic-routes), `params` contains the route parameters. If the page name is `[id].js` , then `params` will look like `{ id: ... }`.
...
- `preview`: `preview` is `true` if the page is in the [Preview Mode](/advanced-features/preview-mode) and `false` otherwise.
- `previewData`: The [preview](/advanced-features/preview-mode) data set by `setPreviewData`.
