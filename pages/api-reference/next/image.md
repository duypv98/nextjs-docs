...
> **Note: This is API documentation for the Image Component and Image Optimization. For a feature overview and usage information for images in Next.js, please see [Images](/basic-features/image-optimization).**
...
1. A [statically imported](/basic-features/image-optimization#local-images) image file, or
...
The `width` property is required, except for [statically imported images](/basic-features/image-optimization#local-images), or those with `layout="fill"`.
...
The `height` property is required, except for [statically imported images](/basic-features/image-optimization#local-images), or those with `layout="fill"`.
...
When `blur`, the [`blurDataURL`](#blurdataurl) property will be used as the placeholder. If `src` is an object from a [static import](/basic-features/image-optimization#local-images) and the imported image is `.jpg`, `.png`, `.webp`, or `.avif`, then `blurDataURL` will be automatically populated.
...
- Custom: `loader: 'custom'` use a custom cloud provider by implementing the [`loader`](/api-reference/next/image#loader) prop on the `next/image` component

If you need a different provider, you can use the [`loader`](#loader) prop with `next/image`.

> Images can not be optimized at build time using [`next export`](/advanced-features/static-html-export), only on-demand. To use `next/image` with `next export`, you will need to use a different loader than the default. [Read more in the discussion.](https://github.com/vercel/next.js/discussions/19065)
...
If you know the expected device widths of your users, you can specify a list of device width breakpoints using the `deviceSizes` property in `next.config.js`. These widths are used when the [`next/image`](/api-reference/next/image) component uses `layout="responsive"` or `layout="fill"` to ensure the correct image is served for user's device.
...
You can configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](/basic-features/image-optimization#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](/api-reference/next.config.js/headers) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
The default [loader](#loader) does not optimize SVG images for a few reasons. First, SVG is a vector format meaning it can be resized losslessly. Second, SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](/advanced-features/security-headers).
...
The `layout="raw"` experiment has been moved to a new module. Please use [`next/future/image`](/api-reference/next/future/image) instead.
...
<div class="card">
  <a href="/basic-features/image-optimization">
    <b>Images</b>
