...
- `res.revalidate(urlPath)` - [Revalidate a page on demand](/basic-features/data-fetching/incremental-static-regeneration#on-demand-revalidation) using `getStaticProps`. `urlPath` must be a `string`.
...
To view more examples using types, check out the [TypeScript documentation](/basic-features/typescript#api-routes).
