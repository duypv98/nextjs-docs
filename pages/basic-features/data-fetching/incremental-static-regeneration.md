...
> **Note:** [Middleware](/advanced-features/middleware) won't be executed for On-Demand ISR requests. Instead, call `revalidate()` on the _exact_ path that you want revalidated. For example, if you have `pages/blog/[slug].js` and a rewrite from `/post-1` -> `/blog/post-1`, you would need to call `res.revalidate('/blog/post-1')`.
...
Next, add the secret as an [Environment Variable](/basic-features/environment-variables) to your application. Finally, create the revalidation API Route:
...
When running locally with `next dev`, `getStaticProps` is invoked on every request. To verify your on-demand ISR configuration is correct, you will need to create a [production build](/api-reference/cli#build) and start the [production server](/api-reference/cli#production):
...
Incremental Static Regeneration (ISR) works on [self-hosted Next.js sites](/deployment#self-hosting) out of the box when you use `next start`.
...
<div class="card">
  <a href="/basic-features/data-fetching/get-static-paths">
    <b>Dynamic routing</b>
