...
The React model allows us to deconstruct a [page](/basic-features/pages) into a series of components. Many of these components are often reused between pages. For example, you might have the same navigation bar and footer on every page.
...
If you only have one layout for your entire application, you can create a [Custom App](/advanced-features/custom-app) and wrap your application with the layout. Since the `<Layout />` component is re-used when changing pages, its component state will be preserved (e.g. input values).
...
Inside your layout, you can fetch data on the client-side using `useEffect` or a library like [SWR](https://swr.vercel.app/). Because this file is not a [Page](/basic-features/pages), you cannot use `getStaticProps` or `getServerSideProps` currently.
...
<div class="card">
  <a href="/advanced-features/custom-app">
    <b>Custom App:</b>
