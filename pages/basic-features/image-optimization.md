...
To add an image to your application, import the [`next/image`](/api-reference/next/image) component:

```jsx
import Image from 'next/image'
```

Alternatively, you can import [`next/future/image`](/api-reference/next/future/image) if you need a component much closer to the native `<img>` element:
...
To use a remote image, the `src` property should be a URL string, which can be [relative](#loaders) or [absolute](/api-reference/next/image#remote-patterns). Because Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](/api-reference/next/image#width), [`height`](/api-reference/next/image#height) and optional [`blurDataURL`](/api-reference/next/image#blurdataurl) props manually:
...
> Learn more about [`remotePatterns`](/api-reference/next/image#remote-patterns) configuration.

### Loaders

Note that in the [example earlier](#remote-images), a partial URL (`"/me.png"`) is provided for a remote image. This is possible because of the `next/image` [loader](/api-reference/next/image#loader) architecture.
...
The default loader for Next.js applications uses the built-in Image Optimization API, which optimizes images from anywhere on the web, and then serves them directly from the Next.js web server. If you would like to serve your images directly from a CDN or image server, you can use one of the [built-in loaders](/api-reference/next/image#built-in-loaders) or write your own with a few lines of JavaScript.
...
See more about priority in the [`next/image` component documentation](/api-reference/next/image#priority).
...
2. Explicitly, by including a [`width`](/api-reference/next/image#width) and [`height`](/api-reference/next/image#height) property
3. Implicitly, by using [`layout="fill"`](/api-reference/next/image#layout) which causes the image to expand to fill its parent element.
...
> **Use `layout='fill'`**
>
> The `fill` layout mode allows your image to be sized by its parent element. Consider using CSS to give the image's parent element space on the page, then using the [`objectFit property`](/api-reference/next/image#objectfit) with `fill`, `contain`, or `cover`, along with the [`objectPosition property`](/api-reference/next/image#objectposition) to define how the image should occupy that space.
>
...
> Note: Many of the styling issues listed below can be solved with [`next/future/image`](/api-reference/next/future/image)
...
The image component has several different [layout modes](/api-reference/next/image#layout) that define how it is sized on the page. If the styling of your image isn't turning out the way you want, consider experimenting with other layout modes.
...
The recommended way to style the inner `<img>` is to set the `className` prop on the Image component to the value of an imported [CSS Module](/basic-features/built-in-css-support#adding-component-level-css). The value of `className` will be automatically applied to the underlying `<img>` element.

Alternatively, you can import a [global stylesheet](/basic-features/built-in-css-support#adding-a-global-stylesheet) and manually set the `className` prop to the same name used in the global stylesheet.

You cannot use [styled-jsx](/basic-features/built-in-css-support#css-in-js) because it's scoped to the current component.
...
[**View all properties available to the `next/image` component.**](/api-reference/next/image)
...
The `next/image` component and Next.js Image Optimization API can be configured in the [`next.config.js` file](/api-reference/next.config.js/introduction). These configurations allow you to [enable remote images](/api-reference/next/image#remote-patterns), [define custom image breakpoints](/api-reference/next/image#device-sizes), [change caching behavior](/api-reference/next/image#caching-behavior) and more.

[**Read the full image configuration documentation for more information.**](/api-reference/next/image#configuration-options)
...
<div class="card">
  <a href="/api-reference/next/image">
    <b>next/image</b>
