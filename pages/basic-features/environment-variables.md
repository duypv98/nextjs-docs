...
> This document is for Next.js versions 9.4 and up. If you’re using an older version of Next.js, upgrade or refer to [Environment Variables in next.config.js](/api-reference/next.config.js/environment-variables).
...
This loads `process.env.DB_HOST`, `process.env.DB_USER`, and `process.env.DB_PASS` into the Node.js environment automatically allowing you to use them in [Next.js data fetching methods](/basic-features/data-fetching/overview) and [API routes](/api-routes/introduction).

For example, using [`getStaticProps`](/basic-features/data-fetching/get-static-props):
