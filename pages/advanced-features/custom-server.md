...
> Before deciding to use a custom server, please keep in mind that it should only be used when the integrated router of Next.js can't meet your app requirements. A custom server will remove important performance optimizations, like **serverless functions** and **[Automatic Static Optimization](/advanced-features/automatic-static-optimization).**
...
- `conf`: `object` - The same object you would use in [next.config.js](/api-reference/next.config.js/introduction). Defaults to `{}`
...
> You may also wish to configure the client-side router to disallow client-side redirects to filename routes; for that refer to [`router.beforePopState`](/api-reference/next/router#router.beforePopState).
