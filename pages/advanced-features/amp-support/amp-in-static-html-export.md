...
When using `next export` to do [Static HTML export](/advanced-features/static-html-export) statically prerender pages, Next.js will detect if the page supports AMP and change the exporting behavior based on that.
...
When [`trailingSlash`](/api-reference/next.config.js/trailing-slash) is enabled the exported pages for `pages/about.js` would be:
