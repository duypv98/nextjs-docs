...
`npm run dev` can be replaced with `yarn dev` if you're using Yarn. If you're [changing the port number](/api-reference/cli#development) your application starts on, replace the `3000` in `http://localhost:3000` with the port you're using instead.
...
If you're using `npm run dev` or `yarn dev` (see [Getting Started](/getting-started)) then you should update the `dev` script on your `package.json`:
