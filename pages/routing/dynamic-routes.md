...
Client-side navigations to dynamic routes are handled with [`next/link`](/api-reference/next/link). If we wanted to have links to the routes used above it will look like this:
...
Read our docs for [Linking between pages](/routing/introduction#linking-between-pages) to learn more.
...
- Pages that are statically optimized by [Automatic Static Optimization](/advanced-features/automatic-static-optimization) will be hydrated without their route parameters provided, i.e `query` will be an empty object (`{}`).
