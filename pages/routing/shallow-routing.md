...
Shallow routing allows you to change the URL without running data fetching methods again, that includes [`getServerSideProps`](/basic-features/data-fetching/get-server-side-props), [`getStaticProps`](/basic-features/data-fetching/get-static-props), and [`getInitialProps`](/api-reference/data-fetching/get-initial-props).

You'll receive the updated `pathname` and the `query` via the [`router` object](/api-reference/next/router#router-object) (added by [`useRouter`](/api-reference/next/router#useRouter) or [`withRouter`](/api-reference/next/router#withRouter)), without losing state.
