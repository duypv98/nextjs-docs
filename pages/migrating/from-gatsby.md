...
Both Gatsby and Next support a `pages` directory, which uses [file-system based routing](/routing/introduction). Gatsby's directory is `src/pages`, which is also [supported by Next.js](/advanced-features/src-directory).

Gatsby creates dynamic routes using the `createPages` API inside of `gatsby-node.js`. With Next, we can use [Dynamic Routes](/routing/dynamic-routes) inside of `pages` to achieve the same effect. Rather than having a `template` directory, you can use the React component inside your dynamic route file. For example:
...
With Gatsby, global CSS imports are included in `gatsby-browser.js`. With Next, you should create a [custom `_app.js`](/advanced-features/custom-app) for global CSS. When migrating, you can copy over your CSS imports directly and update the relative file path, if necessary. Next.js has [built-in CSS support](/basic-features/built-in-css-support).

## Links

The Gatsby `Link` and Next.js [`Link`](/api-reference/next/link) component have a slightly different API.
...
Gatsby uses the `graphql` tag to query data in the pages of your site. This may include local data, remote data, or information about your site configuration. Gatsby only allows the creation of static pages. With Next.js, you can choose on a [per-page basis](/basic-features/pages) which [data fetching strategy](/basic-features/data-fetching/overview) you want. For example, `getServerSideProps` allows you to do server-side rendering. If you wanted to generate a static page, you'd export `getStaticProps` / `getStaticPaths` inside the page, rather than using `pageQuery`. For example:
...
Next.js has a built-in [Image Component and Automatic Image Optimization](/basic-features/image-optimization).
...
Instead, use the built-in [`next/image`](/api-reference/next/image) component and [Automatic Image Optimization](/basic-features/image-optimization).

> The `next/image` component's default loader is not supported when using [`next export`](/advanced-features/static-html-export). However, other loader options will work.
...
Most Gatsby examples use `react-helmet` to assist with adding `meta` tags for proper SEO. With Next.js, we use [`next/head`](/api-reference/next/head) to add `meta` tags to your `<head />` element. For example, here's an SEO component with Gatsby:
