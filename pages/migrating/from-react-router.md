...
This guide will help you understand how to transition from [React Router](https://reactrouter.com) to [file-system based](/routing/introduction) routes with Next.js. Using [`next/link`](/api-reference/next/link) and [`next/router`](/api-reference/next/router) will allow you to:
...
With Next.js, you can express the same application structure in the file system. When a file is added to the [`pages`](/basic-features/pages) directory it's automatically available as a route.
...
Rather than using the `:slug` syntax inside your `Route` component, Next.js uses the `[slug]` syntax in the file name for [Dynamic Routes](/routing/dynamic-routes). We can transform this to Next.js by creating two new files, `pages/blog/index.js` (showing all pages) and `pages/blog/[slug].js` (showing an individual post).
...
Next.js has built-in support for [Server-side Rendering](/basic-features/pages#server-side-rendering). This means you can remove any instances of `StaticRouter` in your code.
...
Each file inside your `pages/` directory will be code split into its own JavaScript bundle during the build process. Next.js [also supports](/basic-features/supported-browsers-features#javascript-language-features) ES2020 dynamic `import()` for JavaScript. With it you can import JavaScript modules dynamically and work with them. They also work with SSR.
...
<div class="card">
  <a href="/api-reference/next/link">
    <b>Pages:</b>
